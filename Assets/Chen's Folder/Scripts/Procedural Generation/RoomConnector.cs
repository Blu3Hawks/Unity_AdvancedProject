...
namespace Chen_s_Folder.Scripts.Procedural_Generation
{
    public class RoomConnector
    {
        ...
        private void ConnectRoomsWithMinimumGuarantee()
        {
            ...
            foreach (Room room in _rooms)
            {
                List<Room> sortedByDistance = new List<Room>(_rooms);
        ...
        private void EnqueueNeighbors(Room room, PriorityQueue<Room, float> priorityQueue, HashSet<Room> visitedRoom)
        {
            ...
            foreach (Room neighborRoom in _rooms)
            {
        ...
        private Room FindClosestRoom(Room roomToCheck, System.Predicate<Room> condition)
        {
            ...
            foreach (Room candidate in _rooms)
            {
