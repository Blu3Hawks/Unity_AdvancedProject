...
namespace Chen_s_Folder.Scripts.Enemies
{
    public class RandomEnemySpawner : MonoBehaviour
    {
        ...
        //lists and stuff
        private List<ScriptableObject> _enemyDataList;
        private List<float> _spawnedEnemiesXp;
        private HashSet<Vector2Int> _usedPositions;
        private List<GameObject> _spawnedCrusherEnemies;
        private List<GameObject> _spawnedWarriorEnemies;
        ...
        private List<Vector3> _crusherPatrolPoints1;
        private List<Vector3> _crusherPatrolPoints2;
        private List<Vector3> _warriorPatrolPoints1;
        private List<Vector3> _warriorPatrolPoints2;

        private void Start()
        {
            _enemyDataList = new List<ScriptableObject> { crusherSkeleton, warriorSkeleton };
            _spawnedEnemiesXp = new List<float>();
            _usedPositions = new HashSet<Vector2Int>();
            _spawnedCrusherEnemies = new List<GameObject>();
            _spawnedWarriorEnemies = new List<GameObject>();
            ListOfEnemies = new List<Enemy>();

            _crusherPatrolPoints1 = new List<Vector3>();
            _crusherPatrolPoints2 = new List<Vector3>();
            _warriorPatrolPoints1 = new List<Vector3>();
            _warriorPatrolPoints2 = new List<Vector3>();
        ...
        {
            this._listOfRooms = new List<Room>(listOfRooms);

            // Get a random percentage 60-80 percent of the rooms will have enemies in them.
            float randomPercentage = Random.Range(60f, 80f);
            int amountOfRoomsToSpawnEnemies = Mathf.RoundToInt(this._listOfRooms.Count * (randomPercentage / 100f));

            // Shuffle the list and get a new list with the amount of rooms to spawn enemies.
            this._listOfRooms = ShuffleList(this._listOfRooms);
            this._listOfRooms = this._listOfRooms.GetRange(0, amountOfRoomsToSpawnEnemies);
        }
        ...
        public void SpawnEnemies(int level)
        {
            ...
            while (totalXP > 0 && _listOfRooms.Count > 0)
            {
        ...
        {
            ScriptableObject enemyData = _enemyDataList[Random.Range(0, _enemyDataList.Count)];
            ...
            if (enemyData is EnemyCrusherSkeletonData crusherData)
            {
                ...
                _spawnedCrusherEnemies.Add(enemyInstance.gameObject);
            }
            else if (enemyData is EnemyWarriorSkeletonData warriorData)
            {
                ...
                _spawnedWarriorEnemies.Add(enemyInstance.gameObject);
            }
            else
            {
            ...
            _usedPositions.Add(spawnPosition2D);
            _spawnedEnemiesXp.Add(enemyXP);
            ...
            {
                _crusherPatrolPoints1.Add(patrolPoint1);
                _crusherPatrolPoints2.Add(patrolPoint2);
            }
            else if (enemyData is EnemyWarriorSkeletonData)
            {
                _warriorPatrolPoints1.Add(patrolPoint1);
                _warriorPatrolPoints2.Add(patrolPoint2);
            }
        ...
        public void InitializeEnemyReferences()
        {
            ...
            {
                enemy.InitializeEnemyReferences(_playerTransform);
            }
        }

        private int SpawnRandomEnemy(int remainingXP)
        {
            Room randomRoom = _listOfRooms[Random.Range(0, _listOfRooms.Count)];
        ...
        private Vector2Int GetRandomSpawnPosition(Room room)
        {
            ...
            do
            {
                ...
                attempts++;
            } while (_usedPositions.Contains(spawnPosition) && attempts < maxAttempt);
        ...
        public void SaveEnemyPositions()
        {
            ...
            foreach (GameObject enemy in _spawnedCrusherEnemies)
            {
                DataPersistenceManager.Instance.GameData.crusherSkeletonPositions.Add(enemy.transform.position);
            }

            foreach (GameObject enemy in _spawnedWarriorEnemies)
            {
                DataPersistenceManager.Instance.GameData.warriorSkeletonPositions.Add(enemy.transform.position);
            }

            DataPersistenceManager.Instance.GameData.crusherPatrolPoints1.AddRange(_crusherPatrolPoints1);
            DataPersistenceManager.Instance.GameData.crusherPatrolPoints2.AddRange(_crusherPatrolPoints2);
            DataPersistenceManager.Instance.GameData.warriorPatrolPoints1.AddRange(_warriorPatrolPoints1);
            DataPersistenceManager.Instance.GameData.warriorPatrolPoints2.AddRange(_warriorPatrolPoints2);
        ...
        public void LoadEnemyPositions()
        {
            ...
            for (int i = 0; i < data.crusherSkeletonPositions.Count; i++)
            {
                ...
                _spawnedCrusherEnemies.Add(enemy);
            ...
            for (int i = 0; i < data.warriorSkeletonPositions.Count; i++)
            {
                ...
                _spawnedWarriorEnemies.Add(enemy);
                Enemy enemyScript = enemy.GetComponent<Enemy>();
                ListOfEnemies.Add(enemyScript);
        ...
        public void ClearEnemies()
        {
            ...
            ListOfEnemies.Clear();
            _spawnedCrusherEnemies.Clear();
            _spawnedWarriorEnemies.Clear();
            _usedPositions.Clear();
            _spawnedEnemiesXp.Clear();
            _crusherPatrolPoints1.Clear();
            _crusherPatrolPoints2.Clear();
            _warriorPatrolPoints1.Clear();
            _warriorPatrolPoints2.Clear();
        }
    }
}
